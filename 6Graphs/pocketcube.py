...
def solve2(config): 
    # Return a sequence of moves to solve config, or None if not possible 
    # halfly explore graph using BFS 
    parentone, frontierone = {SOLVED: None}, [SOLVED]
    parenttwo, frontiertwo = {config: None}, [config] 


    def check(frontier_a, parent_b):
        for f in frontier_a:
            if f in parent_b:
                return f
        return None
    
    midstate = check(frontierone, parenttwo) 
    while midstate is None: 
        frontierone = explore_frontier(frontierone, parentone, True) 
        midstate = check(frontierone, parenttwo)
        if midstate:
            break
        frontiertwo = explore_frontier(frontiertwo, parenttwo, True)
        midstate = check(frontiertwo, parentone)
    if midstate:
        pathone = path_to_config(midstate, parentone)
        pathtwo = path_to_config(midstate, parenttwo)
        # print(pathone)
        # print(pathtwo)
        path = pathtwo[:-1] + list(reversed(pathone))
        # print(path)
        # print('Searched %s reachable configurations' % len(parent)) 
        # print(moves_from_path(path))
        return moves_from_path(path)
    return None

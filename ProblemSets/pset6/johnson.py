...
def johnson(n, S):
    ...
    ### bellman ford part
    Vx = [i for i in range(n+1)] 
    Adjx = [[] for v in Vx]
    Wx = [[INF for v in Vx] for v in Vx]
    for u,v,w in S:
        Adjx[u].append(v)
        Wx[u][v] = w
    for v in range(n):
        Adjx[n].append(v)
        Wx[n][v] = 0
    bf = bellman_ford(Adjx, lambda u,v: Wx[u][v], n)


    ### johnsons dijkstra part
    Wh = [[INF for _ in range(n)] for _ in range(n)]
    if bf is None:
        return None
    else:
        d = bf[0]
        for u in range(n):
            for v in range(n):
                # FIX 1: Only reweight if the edge exists!
                if W[u][v] != INF:
                    Wh[u][v] = W[u][v] +  d[u] - d[v]
                    
        for u in range(n):
            dd, pp = dijkstra(Adj, lambda x, y: Wh[x][y], u)
            if dd:
                for v in range(n):
                    if dd[v] != INF:
                        dd[v] = dd[v] - d[u] + d[v]
                        
                # FIX 2: Overwrite the row, don't append to the end!
                D[u] = dd
